...
/*   Updated: 2018/08/30 17:16:42 by tblaudez         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*get_hexa(va_list ap)
{
	...
	if (!list->modif)
		...
		return (ft_uitoabase(va_arg(ap, uintmax_t), 16));
	else if (list->modif & (1 << 3))
...
char	*get_octal(va_list ap)
{
	...
	if (!list->modif)
		...
		return (ft_uitoabase(va_arg(ap, uintmax_t), 8));
	else if (list->modif & (1 << 3))
...
char	*get_unsigned(va_list ap)
{
	...
	if (!list->modif)
		...
		return (ft_uitoabase(va_arg(ap, uintmax_t), 10));
	else if (list->modif & (1 << 3))
...
char	*get_binary(va_list ap)
{
	...
	if (!list->modif)
		...
		return (ft_uitoabase(va_arg(ap, uintmax_t), 2));
	else if (list->modif & (1 << 3))
